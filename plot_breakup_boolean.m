...
function plot_breakup_boolean(T,col,mar,sz,fcol,i_folder,Folder_sufs,cl,plotscatter,mul,ls)
    filename = append(Folder_sufs{i_folder},'.png');
   figure(1)
%    clf
...
c8=1.5; c9=0.5;
 ...
 for i=1:8 % Can you code these numbers I have put them manually
        for j=1:7
            muvecg(i,j) = muvec(kk);
            ohvecg(i,j) = ohvec(kk);
            wevecg(i,j) = wevec(kk);
...
if (plotscatter)
    ...
    s = scatter(vivec,wevec,sz,'filled','Marker',mar, ...
