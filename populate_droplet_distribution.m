...
% clear;
function populate_droplet_distribution(i_folder,Folder_sufs,wes,mus,ts)
    ...
    for i = i_index
        for j = j_index
            for k = k_index
                ...
                t_considered_droplets = []; % This will store time vs droplet count data
                for nw = 1: (numel(w)-1) %Jump between different computational step
                    if w(nw+1) - w(nw) > 1 %more than one droplet
                        ...
                        t_considered_droplets(nw,:) = [(data(nw,2) - t)/te,length(find(droplet_sizes>Vc))]; %The ones which are counted
